...
public class HeaderPolicyCollection : Dictionary<string, IHeaderPolicy>
{
    ...
    ///     The content-type response prefixes to apply the headers to. For example, using
    ///     <c>new [] { "text/" }</c> would cause all document-based headers to be applied only to responses
    ///     returning content-types such as <c>"text/plain"</c>, or <c>"text/html"</c>
    /// </param>
    /// <remarks>
    ///     Generally, headers such as Content-Security-Policy only make sense at the document level, so
