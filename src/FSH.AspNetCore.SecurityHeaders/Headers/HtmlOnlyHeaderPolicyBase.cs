...
public abstract class HtmlOnlyHeaderPolicyBase : HeaderPolicyBase
{
    ...
    {
        if (context.Response.ContentType?.StartsWith("text/html") ?? true) base.EvaluateHttpRequest(context, result);
    }

    /// <inheritdoc />
    protected override void EvaluateHttpsRequest(HttpContext context, CustomHeadersResult result)
    {
        if (context.Response.ContentType?.StartsWith("text/html") ?? true) base.EvaluateHttpsRequest(context, result);
    }
}