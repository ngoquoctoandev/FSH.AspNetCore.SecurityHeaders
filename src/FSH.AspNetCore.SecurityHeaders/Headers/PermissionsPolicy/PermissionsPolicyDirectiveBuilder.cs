...
public abstract class PermissionsPolicyDirectiveBuilder : PermissionsPolicyDirectiveBuilderBase
{
    ...
    /// </summary>
    internal bool BlockResources { get; set; }

    /// <summary>
    ///     If true, all sources are allowed.
    /// </summary>
    internal bool AllowAllResources { get; set; }
    ...
    internal override string Build()
    {
        ...
        if (BlockResources) return $"{Directive}=()";

        if (AllowAllResources) return $"{Directive}=*";

        if (Sources.Count == 1) return $"{Directive}={Sources.FirstOrDefault()}";
