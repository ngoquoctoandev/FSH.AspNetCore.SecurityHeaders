using FSH.AspNetCore.SecurityHeaders.Headers.ContentSecurityPolicy;
...
public static class ContentSecurityPolicyHeaderExtensions
{
    ...
    public static HeaderPolicyCollection AddContentSecurityPolicy(this HeaderPolicyCollection policies, Action<CspBuilder> configure, bool asReportOnly = false) => policies.ApplyPolicy(ContentSecurityPolicyHeader.Build(configure, asReportOnly));
    ...
    /// <param name="configure">Configure the CSP</param>
    /// <returns>The <see cref="HeaderPolicyCollection" /> for method chaining</returns>
    public static HeaderPolicyCollection AddContentSecurityPolicyReportOnly(this HeaderPolicyCollection policies, Action<CspBuilder> configure) => policies.ApplyPolicy(ContentSecurityPolicyHeader.Build(configure, true));
}