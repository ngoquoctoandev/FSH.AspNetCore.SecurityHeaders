...
public static class ExpectCTHeaderExtensions
{
    ...
    ///     Specifies the number of seconds after reception of the Expect-CT header field during
    ///     which the user agent should regard the host from whom the message was received as a known Expect-CT host.
    /// </param>
    ...
    public static HeaderPolicyCollection AddExpectCTNoEnforceOrReport(
        ...
        AddExpectCT(policies, maxAgeInSeconds, string.Empty, false, _excludedHosts);
    ...
    public static HeaderPolicyCollection AddExpectCTEnforceOnly(
        this HeaderPolicyCollection policies, int maxAgeInSeconds) =>
        AddExpectCT(policies, maxAgeInSeconds, string.Empty, true, _excludedHosts);
    ...
    {
        if (string.IsNullOrWhiteSpace(reportUri)) throw new ArgumentException("Uri may not be null or empty", nameof(reportUri));

        return AddExpectCT(policies, maxAgeInSeconds, reportUri, false, _excludedHosts);
    }
        ...
        this HeaderPolicyCollection policies, int maxAgeInSeconds, string reportUri)
    {
        ...
        return AddExpectCT(policies, maxAgeInSeconds, reportUri, true, _excludedHosts);
    }
    ...
    /// <param name="reportUri">Specifies the URI to which the user agent should report Expect-CT failures.</param>
    /// <param name="enforce">
    ///     If true, signals to the user agent that compliance with the Certificate Transparency policy should be enforced
    ...
    /// <remarks>
    ///     When both the enforce directive and the report-uri directive are present, the configuration is referred to as an "enforce-and-report" configuration, signalling
    ///     to the user agent both that compliance to the Certificate Transparency policy should be enforced and that violations should be reported.
    /// </remarks>
    /// <returns>The <see cref="HeaderPolicyCollection" /> for method chaining</returns>
