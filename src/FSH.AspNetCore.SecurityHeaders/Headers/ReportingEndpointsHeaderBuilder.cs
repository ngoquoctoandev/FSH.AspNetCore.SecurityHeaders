...
public class ReportingEndpointsHeaderBuilder
{
    ...
    /// </summary>
    /// <param name="name">
    ///     The name of the endpoint. This value is used to identify the endpoint
    ///     in other security headers, such as the <c>Content-Security-Policy</c> <c>report-to</c> directive
    /// </param>
    /// <param name="url">The url for the endpoint. Must be a valid, absolute, path</param>
    /// <returns>The <see cref="ReportingEndpointsHeaderBuilder" /> for method chaining</returns>
    public ReportingEndpointsHeaderBuilder AddEndpoint(string name, string url)
    {
        ...
        if (!Uri.TryCreate(url, UriKind.Absolute, out var uri) || !IsValid(uri)) throw new ArgumentException($"The provided url {url} was not a valid absolute URL", nameof(url));
    ...
    public ReportingEndpointsHeaderBuilder AddEndpoint(string name, Uri url)
    {
        ...
        if (!IsValid(url)) throw new ArgumentException($"The provided url '{url}' was not a valid absolute URL", nameof(url));
