using FSH.AspNetCore.SecurityHeaders.Headers;
using Microsoft.AspNetCore.Http;

namespace FSH.AspNetCore.SecurityHeaders.Infrastructure;
...
public class CustomHeaderService : ICustomHeaderService
{
    ...
    /// <returns>
    ///     A <see cref="CustomHeadersResult" /> which contains the result of policy evaluation and can be
    ...
    public virtual void ApplyResult(HttpResponse response, CustomHeadersResult result)
    {
        ...
        foreach (var headerValuePair in result.SetHeaders) headers[headerValuePair.Key] = headerValuePair.Value;

        foreach (var header in result.RemoveHeaders) headers.Remove(header);
    }
}